...
public class GameUIManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        GCRectTransform = gameCanvas.GetComponent<RectTransform>();
        Debug.Log("Game Canvas Rect Transform position: " + GCRectTransform.position);
    ...
    {
        CGRectTransform = cardGridPanel.GetComponent<RectTransform>();
        // Get layout and set grid size
        FlexibleGridLayoutGroup layoutGroup = cardGridPanel.GetComponent<FlexibleGridLayoutGroup>();
        Debug.Log("Flexible Grid Layout Group: " + layoutGroup.transform.position);
        ...
        for (int i = 0; i < shuffledNumbers.Length; i++)
        {
            ...
            // newCard.transform.position = CGRectTransform.transform.localPosition;

            // the cards are all instantiated in the same position, however they should be in a grid layout
            // after changing the transform of the instantiated cards to be that of the card grid panel,
            // there seems to a grid layout but the grid is not centered on the player's screen, why is that?
            //
